...
local function simplify_path(points)
  local marked = {}
  marked[1] = true
  marked[#points] = true
  ...
  while #stack > 0 do
    ...
    if max_dist > prefs.ink_tolerance then
      marked[index] = true
...
function INKTOOL:mouseButton(button, modifiers, press)
  ...
  obj:set("linecap", "round")
  if path_len(self.v) < prefs.ink_dot_length then
    -- pen was pressed and released without no or very little movement
...
function INKTOOL:mouseMove()
  ...
  if (v1-v2):len() > prefs.ink_min_movement then
...
function MODEL:createText(mode)
  ...
  d:add("text", "text", { syntax="latex", focus=true,
			  spell_check=prefs.spell_check }, 0, 1, 1, 2)
...
function MODEL:createParagraph(pos, width, pinned)
  ...
  local d = ipeui.Dialog(self.ui:win(), "Create text object")
  d:add("label", "label", { label="Enter latex source" }, 1, 1, 1, 2)
