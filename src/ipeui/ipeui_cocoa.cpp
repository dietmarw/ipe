...
static CGContextRef ipeGetCGContext()
{
...
{
  return std::move(std::string(N2C([[tv textStorage] string])));
}
...
bool PDialog::buildAndRun(int w, int h)
{
  ...
  for (int i = 0; i < int(iElements.size()); ++i) {
    ...
    if (m.row < 0) {
      ...
      ctrl = b;
      if (m.flags & EAccept)
	[iPanel setDefaultButtonCell:[b cell]]; // changed rendering
    } else {
      switch (m.type) {
      case ELabel:
	{
...
NSImage *colorIcon(double red, double green, double blue, int pixels)
{
  NSImage *icon = [NSImage imageWithSize:NSMakeSize(pixels, pixels)
      ...
      CGContextRef myContext = ipeGetCGContext();
